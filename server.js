...
async function requireAccount(req, res) { const userId = userFromRequest(req); if (!userId) { error(res, 401, 'Please sign in to continue.', 'UNAUTHENTICATED'); return null; } const allUsers = await users(); const account = allUsers.find((user) => user.id === userId); if (!account) { error(res, 401, 'Your session has expired. Please sign in again.', 'UNAUTHENTICATED'); return null; } learningDefaults(account); return { allUsers, account }; }
...
async function api(req, res, url) {
  ...
  if (method === 'POST' && url.pathname === '/api/auth/register') {
    ...
    const allUsers = await users(); if (allUsers.some((user) => String(user.email || '').toLowerCase() === body.email.trim().toLowerCase())) return error(res, 409, 'An account already exists for this email. Please log in.');
    ...
    record(account, 'account_created', 'Learning profile created'); allUsers.push(account); await saveUsers(allUsers); newSession(res, account.id); return json(res, 201, { user: cleanUser(account) });
  }
  if (method === 'POST' && url.pathname === '/api/auth/login') {
    ...
    const body = await readBody(req); const allUsers = await users(); const account = allUsers.find((user) => user.email === String(body.email || '').trim().toLowerCase()); if (!account || !isPasswordValid(String(body.password || ''), account)) return error(res, 401, 'Email or password is incorrect.', 'INVALID_CREDENTIALS'); learningDefaults(account); newSession(res, account.id); return json(res, 200, { user: cleanUser(account) });
  }
  ...
  const scope = await requireAccount(req, res); if (!scope) return; const { allUsers, account } = scope;
...
http.createServer(async (req, res) => { const url = new URL(req.url, `http://${req.headers.host || 'localhost'}`); try { if (url.pathname.startsWith('/api/')) await api(req, res, url); else await staticFile(req, res, url); } catch (err) { console.error(err); error(res, 500, 'The server could not complete that request.', 'SERVER_ERROR'); } }).listen(PORT, () => console.log(`studentबंधु is running at http://localhost:${PORT}`));
